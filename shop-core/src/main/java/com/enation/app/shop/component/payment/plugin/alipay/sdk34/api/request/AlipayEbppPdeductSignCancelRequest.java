...
import com.enation.app.shop.component.payment.plugin.alipay.sdk34.api.response.AlipayEbppPdeductSignCancelResponse;
...
public class AlipayEbppPdeductSignCancelRequest implements AlipayRequest<AlipayEbppPdeductSignCancelResponse> {
	...
	* 支付宝代扣协议ID
	...
	/** 
	* 需要用户首先处于登录态，然后访问https://ebppprod.alipay.com/deduct/enterMobileicPayPassword.htm?cb=自己指定的回跳连接地址,访问页面后会进到独立密码校验页，用户输入密码校验成功后，会生成token回调到指定的回跳地址，如果设置cb=www.baidu.com则最后回调的内容是www.baidu.com?token=312314ADFDSFAS,然后签约时直接取得地址后token的值即可
	 */
