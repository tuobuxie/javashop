...
//				shortship, shortship, shipmoney, discount, orderid);
	...
	@Override
	public List<Order> listPayOrders(Integer payStatus) {
		String sql = "select * from es_order where status = 1 and pay_status = ?";	
		return daoSupport.queryForList(sql,Order.class,payStatus);
	}
	...
	@Override
	public void updatePayState(String sn,double payMoney, int orderState,int payState) {
		this.daoSupport.execute("update es_order set pay_status=?,status=?,paymoney=? where sn=? and pay_status = 0",
				payState,orderState,payMoney,sn);
	}
